      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        <div className="mb-6 text-center">
          <h2 className="text-2xl font-extrabold tracking-tight md:text-3xl">
            <span className="bg-[#FBAC18] px-2 py-1 rounded font-black text-black">JACKET</span>
            <span className="ml-2">: The Gold Standard</span>
          </h2>
        </div>

        <div className="overflow-x-auto rounded-lg border border-[#121212] bg-white shadow">
          <table className="min-w-[680px] w-full">
            <thead>
              <tr>
                <th className="w-1/2 bg-[#FBAC18] px-4 py-3 text-left text-sm font-bold text-black md:text-base">
                  JACKET SUNSCREEN
                </th>
                <th className="w-1/2 bg-black px-4 py-3 text-left text-sm font-bold text-white md:text-base">
                  OTHER SUNSCREENS
                </th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.label} className="border-t border-[#121212]">
                  <td className="bg-[#FFF8EB] px-4 py-3 text-[11px] font-bold">
                    <div className="flex items-center gap-3 text-neutral-900">
                      <GoldCheckIcon />
                      <span>{row.label}</span>
                    </div>
                  </td>
                  <td className="bg-[#E0E0E0] px-4 py-3 text-[11px] font-bold">
                    <div className={`flex items-center gap-3 text-neutral-700 ${row.rightHas ? 'opacity-100' : 'opacity-60'}`}>
          ...
          <div className="flex items-center justify-center border-t border-neutral-200 bg-white p-3">
            <button
              type="button"
              onClick={() => setExpanded((v) => !v)}
              className="text-sm font-semibold text-neutral-900 underline md:text-base"
            >
